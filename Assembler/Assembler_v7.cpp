...
int FindCommandS(char* commName, int* number, int* isJumper) {
	assert(commName != NULL);
	assert(number != NULL);
	assert(isJumper != NULL);

#define DEF_CMD(name, num, isJmp)\
...
int ConvertString(char* stringS, char* stringI, int* strLength, int* isJumper) {
	assert(stringS != NULL);
	assert(stringI != NULL);
	assert(strLength != NULL);

	int commTNum = 0;
	if (FindCommandS(stringS, &commTNum, isJumper) == 1) {
		return 1;
	}
...
int CreateMshnFile(FILE* fin, FILE* fout, const int stringMaxLength) {
	...
	do {
		...
		int isJumper = 0;
		if (curStringS[0] != labelCh) {
			if (ConvertString(curStringS, curStringI, &curStringILength, &isJumper) != 0) {
				return 1;
			}
